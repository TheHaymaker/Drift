...
/**
 * git-poem-parser v2
 * Produces atom-level data from git history with character-granularity diffs.
 *
 * Usage: node git-poem-parser.js <repo-path> <file> [output.json]
 */
const { execSync } = require("child_process");
const fs = require("fs");

const repoPath = process.argv[2] || "./poem-repo";
const filePath = process.argv[3] || "poem.txt";
const outputPath = process.argv[4] || "./poem-atoms.json";

function git(cmd) {
  return execSync(`git ${cmd}`, { cwd: repoPath, encoding: "utf-8" }).trim();
}

function getCommits() {
  return git(`log --reverse --format="%H|%ai|%s" -- ${filePath}`)
    .split("\n").map(line => {
...
function getFileAtCommit(hash) {
  try { return git(`show ${hash}:${filePath}`); } catch { return ""; }
}
...
function lcs(a, b) {
  ...
  let i = m, j = n;
  while (i > 0 && j > 0) {
    if (a[i-1] === b[j-1]) { res.unshift({ ai: i-1, bi: j-1, v: a[i-1] }); i--; j--; }
...
function wordDiff(oldLine, newLine) {
  const ow = oldLine.split(/(\s+)/).filter(Boolean);
  const nw = newLine.split(/(\s+)/).filter(Boolean);
  const owWords = ow.filter(w => w.trim());
  const nwWords = nw.filter(w => w.trim());
  ...
  // Pair delete/insert that look like morphs
  const result = [];
  let idx = 0;
...
function buildFrames(commits) {
  const snapshots = commits.map(c => {
    const content = getFileAtCommit(c.hash);
...
const data = buildFrames(commits);
fs.writeFileSync(outputPath, JSON.stringify(data, null, 2));

console.log(`\nWrote ${outputPath}`);
for (let i = 0; i < data.frames.length; i++) {
  const f = data.frames[i];
  const born = f.filter(a => a.status === "born").length;
  const kept = f.filter(a => a.status === "kept").length;
  const morphed = f.filter(a => a.status === "morphed");
  const died = f.filter(a => a.status === "died").length;
  console.log(`  ${data.commits[i].hash} | born:${born} kept:${kept} morphed:${morphed.length} died:${died}`);
  for (const m of morphed) {
    console.log(`    morph: "${m.morphFrom}" → "${m.text}"  charDiff: ${JSON.stringify(m.charDiff)}`);
  }
}
