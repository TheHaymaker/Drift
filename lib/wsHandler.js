...
function handleWebSocketUpgrade(server, sessions, sessionMiddleware) {
  ...
  wss.on("connection", async (ws, req) => {
    ...
    // Parse session from upgrade request cookie (must await — store read is async)
    try {
      await new Promise((resolve, reject) => {
        const dummyRes = Object.create(http.ServerResponse.prototype);
        dummyRes.getHeader = () => {};
        dummyRes.setHeader = () => dummyRes;
        dummyRes.writeHead = () => dummyRes;
        sessionMiddleware(req, dummyRes, (err) => {
