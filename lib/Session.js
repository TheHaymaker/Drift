...
class Session {
  ...
  enqueueGitOp(asyncFn) {
    this.commitQueue = this.commitQueue
      .then(asyncFn)
      .catch(err => console.error('[Session] git operation failed for doc', this.docId, ':', err.message, err.stack));
    return this.commitQueue;
  }

  async handleUpdate(ws, content) {
    await GitOps.writeFile(this.repoPath, this.filename, content).catch(err =>
      console.error('[Session] writeFile error for doc', this.docId, ':', err.message, err.stack)
    );

    clearTimeout(this.pauseTimer);
    this.pauseTimer = setTimeout(() => {
      if (content !== this.lastContent) {
        const contentSnapshot = this.lastContent; // capture before async gap
        const commitMsg = generateMessage(contentSnapshot, content);
  ...
  async flush() {
    clearTimeout(this.pauseTimer);
    const contentToFlush = this.lastContent; // capture immediately
    const diskContent = await GitOps.readFile(this.repoPath, this.filename);
    if (diskContent !== contentToFlush) {
      await this.enqueueGitOp(async () => {
        const msg = generateMessage(contentToFlush, diskContent);
