...
async function readFile(repoPath, filename) {
  try {
    ...
    console.error('[GitOps] readFile failed:', err.message);
    return "";
  }
}
...
async function commitFile(repoPath, filename, message) {
  try {
    ...
    console.error('[GitOps] commit failed:', err.message);
    return null;
  }
}
...
async function getLog(repoPath, filename, max = 200) {
  try {
    ...
    console.error('[GitOps] log failed:', err.message);
    return [];
  }
}
...
async function getFileAt(repoPath, hash, filename) {
  try {
    ...
    console.error('[GitOps] getFileAt failed:', err.message);
    return "";
  }
}
...
async function getWordDiff(repoPath, hashA, hashB, filename) {
  try {
    ...
    console.error('[GitOps] getWordDiff failed:', err.message);
    return "";
  }
}
...
async function getStructuredDiff(repoPath, hashA, hashB, filename) {
  try {
    ...
    console.error('[GitOps] getStructuredDiff failed:', err.message);
    return [];
  }
}
...
async function renameFile(repoPath, oldFilename, newFilename) {
  await git(repoPath, ["mv", oldFilename, newFilename]);
  await git(repoPath, ["commit", "-m", `rename: ${oldFilename} \u2192 ${newFilename}`]);
}
